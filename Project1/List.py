...
class List:
    ...
    def insert(self, position: Node, val: Generic[T], num: int = 1) -> Node:
        ...
        """
        new = Node(val, position, position.prev)
    ...
    def remove(self, val: Generic[T]) -> None:
        ...
        def remove_node(node: Node) -> Node:
            """remove helper"""
            if node.val == val and node.next is root:
                node.prev.next = node.next
                node.next.prev = node.prev
                return node.next
            elif node.val == val and node.next is not root:
                node.prev.next = node.next
                node.next.prev = node.prev
                return remove_node(node.next)
            elif node.next is root:
                return node.next
            else:
                return remove_node(node.next)
    ...
    def remove_if(self, pred: Callable[[T], bool]) -> None:
        ...
        def remove_node_if(node: Node) -> Node:
            """remove_if helper"""
            if pred(node.val) is True and node.next is root:
                node.prev.next = node.next
                node.next.prev = node.prev
                return node.next
            elif pred(node.val) is True and node.next is not root:
                node.prev.next = node.next
                node.next.prev = node.prev
...
def fix_playlist(lst: List) -> bool:
    ...
    root = lst.node
    if lst.node.next is root:
        return True
    elif lst.node.next is None:
